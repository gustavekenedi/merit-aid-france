rd_observable_fct <- function() {
  
  rds_file <- here(glue("out/figures/raw_data/{covariate}_observables_graph_dt.rds"))
  
  data_graph <- read_rds(rds_file)[[1]]
  data_donut <- read_rds(rds_file)[[2]]
  pred <- read_rds(rds_file)[[3]]
